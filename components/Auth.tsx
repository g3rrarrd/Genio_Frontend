  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok || response.status === 201) {
        // MAPEAMOS AQUÍ: Convertimos identificador (DB) a name (Frontend)
        const userToSave = {
          ...data,
        ...
        onSuccess(userToSave); // Esto activará handleAuthSuccess en App.tsx
      } else {
  ...
  return (
    <div className="min-h-screen flex flex-col p-6 max-w-md mx-auto bg-background-dark soccer-pattern">
      <header className="pt-12 text-center mb-12">
        <button 
          onClick={onBack}
          className="absolute top-6 left-6 w-10 h-10 flex items-center justify-center rounded-full bg-white/5 border border-white/10"
        >
          <span className="material-symbols-outlined text-white">arrow_back</span>
        </button>
        <h1 className="text-4xl font-black italic tracking-tighter text-white uppercase leading-none">
          NUEVO <br/>
          <span className="text-primary drop-shadow-[0_0_15px_rgba(19,236,91,0.4)]">FICHAJE</span>
        </h1>
        <p className="text-white/40 text-xs font-bold uppercase mt-4 tracking-wider px-8">Crea tu perfil de crack para empezar a competir</p>
      </header>

      <form onSubmit={handleSubmit} className="flex-1 space-y-5">
        {error && (
          <div className="bg-red-500/10 border border-red-500/50 text-red-500 p-3 rounded-xl text-center text-[10px] font-black uppercase">
            {error}
          </div>
        )}
        ...
        <div className="space-y-2">
          <label className="text-[10px] font-black uppercase text-white/40 tracking-[0.2em] ml-2">Tu Grito de Guerra (Descripción)</label>
          <div className="relative">
            ...
            <textarea 
              ...
              className="w-full bg-card-bg border border-white/10 rounded-2xl py-4 pl-12 pr-4 text-white focus:outline-none focus:border-primary/50 transition-colors resize-none"
            />
          </div>
        </div>
        ...
        <div className="pt-4">
            <button 
                type="submit"
                disabled={isLoading}
                className={`w-full bg-primary text-background-dark font-black py-5 rounded-2xl shadow-[0_10px_30px_rgba(19,236,91,0.4)] active:scale-95 transition-all flex items-center justify-center gap-3 uppercase tracking-tighter text-xl italic ${isLoading ? 'opacity-50 cursor-wait' : ''}`}
            >
